...
void Conv_Device::im2col(const Vector& image, Matrix& data_col) {
...
void Conv_Device::forward(const Matrix& bottom) {
    ...
    const int kernel_size = height_kernel;
	...
	std::cout << "\t Time: " << duration_layer << " ms" << std::endl;
}

// col2im, used for grad_bottom
// data_col size: Matrix (hw_out, hw_kernel * channel_in)
// image size: Vector (height_in * width_in * channel_in)
void Conv_Device::col2im(const Matrix& data_col, Vector& image) {
  int hw_in = height_in * width_in;
  int hw_kernel = height_kernel * width_kernel;
  int hw_out = height_out * width_out;
  // col2im
...
void Conv_Device::backward(const Matrix& bottom, const Matrix& grad_top) {
  int n_sample = bottom.cols();
  grad_weight.setZero();
  grad_bias.setZero();
...
void Conv_Device::update(Optimizer& opt) {
...
std::vector<float> Conv_Device::get_parameters() const {
...
void Conv_Device::set_parameters(const std::vector<float>& param) {
...
std::vector<float> Conv_Device::get_derivatives() const {
